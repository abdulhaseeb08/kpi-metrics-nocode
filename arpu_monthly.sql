...
, arpu as (
select TO_CHAR(subscription_start, 'Month') as month_name, 
       EXTRACT(YEAR FROM cast(subscription_start as date)) "year", 
       sum(amount) as total_revenue, 
       count(distinct customer) as total_customers
from revenue_monthly
group by TO_CHAR(subscription_start, 'Month'), EXTRACT(YEAR FROM cast(subscription_start as date))
)
select month_name, "year", total_revenue/total_customers as arpu_monthly
from arpu
with single_invoice as (
	select * from (
		select i.id, 
		  ...
		  left join invoice_line_items_period ilipd 
...
(
   ...
   left JOIN   PUBLIC.invoice_line_items_price ilip
   ...
   on p.id = pr.product
   left join invoice_line_items_period ilipd on ili."_airbyte_invoice_line_items_hashid" =ilipd."_airbyte_invoice_line_items_hashid"
   left join invoice_line_items_plan ilipp on ilipp."_airbyte_invoice_line_items_hashid" = ili."_airbyte_invoice_line_items_hashid"
   ...
   left join   PUBLIC.invoice_line_items_price ilip on ili."_airbyte_invoice_line_items_hashid" =ilip."_airbyte_invoice_line_items_hashid"
   left join   PUBLIC.products p on  ilip.product = p.id
   left join public.prices pr
   on p.id = pr.product
   left join invoice_line_items_period ilipd on ili."_airbyte_invoice_line_items_hashid" =ilipd."_airbyte_invoice_line_items_hashid"
   left join invoice_line_items_plan ilipp on ilipp."_airbyte_invoice_line_items_hashid" = ili."_airbyte_invoice_line_items_hashid"
